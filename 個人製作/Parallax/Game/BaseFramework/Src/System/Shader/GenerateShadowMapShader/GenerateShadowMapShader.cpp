...
{
	// プレイヤーの位置
	Vec3 PlayerPos = Scene::GetInstance().GetPlayerMatrix().GetTranslation();
	PlayerPos.Normalize();
	...
	// ライトのビュー行列を適当に作成
	Matrix mV;
	mV.LookTo({ 0.5f, -1, 0.5f }, { 0, 1, 0 });
	Matrix mTrans;
	//mTrans.CreateTranslation(0, 0, -20);
	mTrans.CreateTranslation(PlayerPos.x, PlayerPos.y, -20);
	mV = mTrans * mV;
	mV.Inverse();

	// ライトの射影行列を適当に作成　10mx10mの正射影行列
	Matrix mP;
	mP.CreateProjection_Orthographic(10, 10, 0, 100);

	// ビュー*射影をセット
	m_cb0.Work().mLightVP = mV * mP;

	// ModelShaderで使用するためライトの定数バッファに行列を入れておく
