...
std::shared_ptr<GLTFModel> LoadGLTFModel(const std::string& path)
{
	...
	std::shared_ptr<GLTFModel>	destModel = std::make_shared<GLTFModel>();
	...
	{
		// 行列計算用 再帰関数
		std::function<void(GLTFNode*, const Math::Matrix*)> rec = [&rec, &destModel](GLTFNode* node, const Math::Matrix* parentMat)
		{
			if (parentMat) {
